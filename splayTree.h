...
struct SplayTree{
	...
	pair<int, int> split(int j, const function<bool(int j)>& comp){
		if(!j) return {0, 0};
		assert(!node[j].parent);
	...
	int add(int i){
		up(i);
		pair<int, int> p = split(root, [&](int j){return node[j] < node[i];});
	...
	int lower_bound(Node v){ /************* add end ************/
		pair<int, int> p = split(root, [&](int j){return node[j] < v;});
	...
	int upper_bound(Node v){
		pair<int, int> p = split(root, [&](int j){return node[j] < v || node[j] == v;});
		int t = leftmost(p.second);
		root = merge(p.first, t);
		return t;	
	}
